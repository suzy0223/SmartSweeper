...
const int NumPeopleVerts = 4;
const SPoint people[NumPeopleVerts] = {SPoint(1,1),
SPoint(1,-1),
SPoint(-1,-1),
SPoint(-1,1)};
...
cyClient(CParams::WindowHeight)
{
	...
	m_PeoplePen = CreatePen(PS_SOLID, 1, RGB(255, 255, 0));
	m_OldPen	= NULL;
	...
	if (m_iTicks++ < CParams::iNumTicks)
	{
		for (i=0; i<m_NumSweepers; ++i)
		{
			...
			if (GrabHit2 >= 0)
			{
				...
				//people found so replace the people with another at a random 
				//position
...
void CController::Render(HDC surface)
{
	...
	if (!m_bFastRender)
	{
		...
		for (i=0; i<m_NumPeople; ++i)
		{
			...
			for (vert=1; vert<peopleVB.size(); ++vert)
			{
